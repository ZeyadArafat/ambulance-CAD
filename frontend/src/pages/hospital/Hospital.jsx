...
export default function Hospital() {
  ...
  return <div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_340px] grid-rows-[auto_minmax(0,1fr)] gap-3 overflow-auto p-4">
    ...
    <Panel title="INBOUND PATIENTS" subtitle="LIVE PRE-ARRIVAL NOTIFICATIONS" className="flex min-h-0 flex-col overflow-hidden"><div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">{error && <div className="border border-[#7F1D1D] bg-[#29151A] p-2 text-[11px] text-[#FCA5A5]">{error}</div>}{!loading && !selectedInbound.length && <div className="p-3 text-[11px] text-[#7E8A9A]">No active inbound dispatches for this facility.</div>}{selectedInbound.map((item) => { const isAcknowledged = acknowledged.includes(item.dispatch_id) || Boolean(item.destination?.acknowledged_at); return <div key={item.dispatch_id} className="flex items-center justify-between gap-3 border border-[#222B3A] bg-[#0F141D] p-3"><div className="min-w-0"><div className="flex items-center gap-2"><b className="text-xs">{item.incident_id}</b><PriorityBadge priority={item.priority} /></div><p className="mt-2 truncate text-[11px] text-[#AAB4C3]">{item.ambulance_code || 'Assigned ambulance'} · {item.dispatch_status}</p><p className="mt-1 text-[10px] text-[#7E8A9A]">ETA {item.eta_minutes ?? '—'} MIN · {item.hospitalName}</p></div><ActionButton icon={Check} variant={isAcknowledged ? 'ghost' : 'primary'} onClick={() => acknowledge(item)}>{isAcknowledged ? 'ACKED' : 'ACCEPT'}</ActionButton></div> })}</div></Panel>
