...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([getUsers(), getRoles(), getAuditLog(), getStations(), getZones(), getVehicles(), getTriageProtocols(), getDispatchRecommendationProtocol()])
      .then(([userResponse, roleResponse, auditResponse, stationResponse, zoneResponse, vehicleResponse, triageResponse, recommendationResponse]) => {
...
function Input({ label, value, onChange }) { return <label className="block"><span className="cad-label">{label}</span><input required value={value} onChange={(event) => onChange(event.target.value)} className="cad-input" /></label> }
