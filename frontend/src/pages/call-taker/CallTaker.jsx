...
export default function CallTaker() {
  ...
  const addSupplementaryNote = () => {
    ...
    appendNote(noteIncident, note.trim())
  ...
  return <div className="h-full w-full overflow-y-auto bg-[#0B0F14] p-4">
    <div className="grid min-h-full grid-cols-[minmax(0,3fr)_minmax(370px,2fr)] gap-4">
      <form onSubmit={submitIncident} className="flex min-h-0 flex-col border border-[#222B3A] bg-[#121620]">
        <header className="flex min-h-14 shrink-0 items-center justify-between border-b border-[#222B3A] px-4"><div><h1 className="text-sm font-bold tracking-[0.1em] text-[#F5F7FA]">NEW INCIDENT INTAKE</h1><p className="mt-1 text-[10px] text-[#7E8A9A]">Create a mock CAD incident and place it in the active dispatch queue.</p></div><span className="border border-[#222B3A] bg-[#0E131B] px-2 py-1 font-mono text-[9px] font-bold text-[#7E8A9A]">NEW</span></header>
        <div className="flex min-h-0 flex-1 flex-col p-4">
          ...
          <div className="mt-4 flex min-h-12 items-center gap-3 border-t border-[#222B3A] pt-3"><div className="mr-auto min-w-0 text-[10px]" aria-live="polite">{createdIncident ? <span className="inline-flex items-center gap-1.5 text-[#86EFAC]"><CheckCircle2 size={14} />{createdIncident.id} submitted to dispatch as {createdIncident.priority}.</span> : <span className="text-[#7E8A9A]">Required fields show inline feedback when missing.</span>}</div><ActionButton type="button" icon={RotateCcw} onClick={clearForm}>CLEAR FORM</ActionButton><ActionButton type="submit" icon={Send} variant="primary" className="min-w-[210px]">SUBMIT TO DISPATCH</ActionButton></div>
        </div>
      </form>
